...
On Kubernetes, connect a Container directly to the Secret to inject each data
key through a Kubernetes secret reference. A connection named `credentials`
creates variables such as `CONNECTION_CREDENTIALS_USERNAME` and
`CONNECTION_CREDENTIALS_PASSWORD` in regular and init containers. Set
`disableDefaultEnvVars: true` to disable automatic injection; explicitly
declared container environment variables take precedence.

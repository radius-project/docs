...
Other Radius resources, such as [containers]({{< ref "guides/applications/containers" >}}), may connect to a Azure SQL resource via connections. When a connection to Azure SQL named, for example, `myconnection` is declared, Radius injects values into environment variables that are then used to access the connected Azure SQL resource:

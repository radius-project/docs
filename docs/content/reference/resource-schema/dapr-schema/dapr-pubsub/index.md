...
Other Radius resources, such as [containers]({{< ref "guides/applications/containers" >}}), may connect to a Dapr pub/sub resource via connections. When a connection to Dapr pub/sub named, for example, `myconnection` is declared, Radius injects values into environment variables that are then used to access the connected Dapr pub/sub resource:

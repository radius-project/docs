...
**Yes**. You can use any Azure resource type by modeling it in Bicep outside the `radius.dev/Application` resource and defining a connection to the resource from a `Container`. See the [connections page]({{< ref appmodel-concept >}})) for more details.

...
| image | The name/tag of the Docker image to build and push at deploy time. When running locally with [`rad app run`]({{< ref rad_application_run >}}) this value is overriden to use the [local registry]({{< ref "dev-environments#local-container-registry" >}}). | `myregistry/myimage` |

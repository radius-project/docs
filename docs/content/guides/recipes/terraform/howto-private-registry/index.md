...
Configure a [Radius Secret Store]({{< ref "/guides/author-apps/secrets/overview" >}}) with both username and personal access token (or password) that you previously created, ensuring they have access to your private git repository. Define the namespace for the cluster that will contain your [Kubernetes Secret](https://kubernetes.io/docs/concepts/configuration/secret/) with the `resource` property. 
...
> Both `username` and `pat` properties are required. `username` refers to your git platform username, and `pat` refers to your personal access token or password.

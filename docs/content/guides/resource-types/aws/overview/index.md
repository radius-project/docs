...
The AWS provider allows you to deploy and connect to AWS resources from a Radius Environment on an EKS cluster. To configure an AWS provider, you can follow the documentation [here]({{< ref "/guides/setup-platform/cloud-providers/aws-provider" >}}).
...
{{% codetab %}}
In the following example, a [Container]({{< ref "guides/resource-types/core-resource-types/containers" >}}) is connecting to an S3 bucket.

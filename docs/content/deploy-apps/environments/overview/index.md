...
Radius environments can be configured with a container runtime, where Radius [containers]({{< ref "/author-apps/containers" >}}) will be run, along with [gateways and routes]({{< ref networking >}}).
...
1. Begin by deploying a compatible [Kubernetes cluster]({{< ref "/operations/kubernetes" >}})

   *Visit the [Kubernetes platform docs]({{< ref "/operations/kubernetes" >}}) for a list of supported clusters and specific cluster requirements.*

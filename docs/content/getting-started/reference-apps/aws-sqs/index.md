...
description: "Learn about how to model a AWS SQS Queue resources in Bicep and them it in a Radius application"
weight: 500
---
...
1. Deploy your application to your environment:
    ...
    ```bash
    rad deploy ./app.bicep -p aws_access_key_id=<AWS_ACCESS_KEY_ID> -p aws_secret_access_key=<AWS_SECRET_ACCESS_KEY> -p aws_region=<REGION> -p queue_name=<SQS_QUEUE_NAME>
    ```
...
1. Port-forward the container to your machine with [`rad resource expose`]({{< ref rad_resource_expose >}}):
    ...
    ```bash
    rad resource expose containers producer -a sqs-emitter-receiver --port 3000
    ```
...
1. Visit [localhost:3000/receive](http://localhost:3000/send) in your browser, this will consume the SQS Queue message and display its contents.

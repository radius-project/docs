...
Within the application resource, the **environmentId** property specifies the environmentId to deploy the application and other resources into. Since this was initialized in the previous step via the CLI, `rad deploy` will automatically inject the `environmentId` parameter into the Bicep file during deployment.

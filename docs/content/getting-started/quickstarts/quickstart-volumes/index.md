...
The `quickstarts/volumes` container will display the status and contents of the `/tmp` directory within the container.
...
1. Once complete, port forward to your container with [`rad resource expose`]({{< ref rad_resource_expose >}}):
   ...
   rad resource expose containers mycontainer -a myapp --port 5000
   ```
1. You should see the contents of `/tmpdir`, showing an empty directory.

...
1. Use the `rad init` command to initialize a new environment into your current kubectl context:
   ...
   Follow the prompts to install the [control plane services]({{< ref architecture >}}), create an [environment resource]({{< ref environments-resource >}}), and create a [local workspace]({{< ref local-workspaces >}}). You will be asked for:

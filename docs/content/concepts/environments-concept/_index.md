...
Once an environment template is defined, developers can get up and running with a prepared development, pre-production, or production environment in minutes. No more waiting for manual provisioning, configuration or inter-team coordination. Simply use the [Recipes]({{< ref "/recipes-section/overview" >}}) linked to your Radius environment and deploy your application.
...
| **Infrastructure provisioning** | Automatically provision infrastructure at deployment time through [Recipes]({{< ref "/recipes-section/overview" >}}).

...
Radius is an open-source project that provides a platform for modeling and deploying applications. Radius is not a paid product or service. Instead, Radius allows you to host your own Radius environments on your own Kubernetes clusters, and deploy applications to any cloud provider or on-premises infrastructure. Radius is providing new value is through Recipes and the Application Graph. Recipes make it easier for developers and platform engineers/IT engineers to collaborate with each other in building cloud native apps. The Application Graph makes it easier to see all of the various components that make up a cloud native application. We think these are additive and unique.
